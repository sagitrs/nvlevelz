...
struct Trie {
    ...
    struct Node {
        ...
        Trie* next_;
        Trie* prev_;
        Node(DType* data, Trie* next, Trie* prev) : data_(data), next_(next), prev_(prev) {}
    } *node_;
...
//            printf("Wait");
    ...
    string FuzzyLeftBoundary(const string& key) {
        Trie* t = FuzzyFind_(key);
        if (t == nullptr) t = Head();
        return t->Key();
    }
    string FuzzyRightBoundary(const string& key) {
        Trie* t = FuzzyFind_(key);
        if (t == nullptr) t = Head();
        assert(t->Data());
        if (t->Next() == nullptr || t->Next() == t) return "";
        t = t->Next();
        return t->Key();
    }
    void Print() {
    ...
    class Iterator {
     public:
      ...
      void Seek(const string& key) {
          ...
          assert(current_ == nullptr || current_->Data());
          if (current_ == nullptr) {
              SeekToFirst();
              if (key.compare(current_->Key()) < 0) return;
              SeekToLast();
              assert(key.compare(current_->Key()) > 0);
          }
      }
